...
import nn.initializers


# Layers

# Each layer constructor function returns an (init_fun, apply_fun) pair, where
#   init_fun: takes an rng key and an example input and returns params
...
def Dense(out_dim, W_init=glorot(), b_init=randn()):
  ...
  def init_fun(rng, x):
    ...
    return W_init(k1, (x.shape[-1], out_dim)), b_init(k2, (out_dim,))
...
def GeneralConv(dimension_numbers, out_chan, filter_shape,
  ...
  def init_fun(rng, x):
    ...
    kernel_shape = [out_chan if c == 'O' else
                    x.shape[lhs_spec.index('C')] if c == 'I' else
                    next(filter_shape_iter) for c in rhs_spec]
    bias_shape = [out_chan if c == 'C' else 1 for c in out_spec]
    bias_shape = tuple(itertools.dropwhile(lambda x: x == 1, bias_shape))
    k1, k2 = random.split(rng)
    return W_init(k1, kernel_shape), b_init(k2, bias_shape)
  def apply_fun(params, x, **kwargs):
    W, b = params
    return lax.conv_general_dilated(x, W, strides, padding, one, one,
                                    dimension_numbers) + b
  return init_fun, apply_fun
...
def GeneralConvTranspose(dimension_numbers, out_chan, filter_shape,
  ...
  def apply_fun(params, x, **kwargs):
    W, b = params
    return lax.conv_transpose(x, W, strides, padding, dimension_numbers) + b
  return init_fun, apply_fun
...
def BatchNorm(axis=(0, 1, 2), epsilon=1e-5, center=True, scale=True,
  ...
  def init_fun(rng, x):
    ...
    k1, k2 = random.split(rng)
    return _beta_init(k1, shape), _gamma_init(k2, shape)
  def apply_fun(params, x, **kwargs):
    beta, gamma = params
...
def elementwise(fun, **fun_kwargs):
  ...
  init_fun = lambda rng, x: ()
  apply_fun = lambda params, x, **kwargs: fun(x, **fun_kwargs)
  return init_fun, apply_fun
...
def _pooling_layer(reducer, init_val, rescaler=None):
  def PoolingLayer(window_shape, strides=None, padding='VALID'):
    ...
    init_fun = lambda rng, x: ()
    def apply_fun(params, x, **kwargs):
      out = lax.reduce_window(x, init_val, reducer, dims, strides, padding)
      return rescale(out, x) if rescale else out
    return init_fun, apply_fun
  return PoolingLayer
...
def Identity():
  ...
  init_fun = lambda rng, x: ()
  apply_fun = lambda params, x, **kwargs: x
  return init_fun, apply_fun
Identity = Identity()
...
def FanOut(num):
  ...
  init_fun = lambda rng, x: ()
  apply_fun = lambda params, x, **kwargs: [x] * num
  return init_fun, apply_fun


def FanInSum():
  """Layer construction function for a fan-in sum layer."""
  init_fun = lambda rng, inputs: ()
...
def serial(*layers):
  ...
  def init_fun(rng, x):
    params = []
    for (init_fun, apply_fun) in zip(init_funs, apply_funs):
      rng, layer_rng = random.split(rng)
      param = init_fun(layer_rng, x)
      x = apply_fun(param, x, rng=layer_rng)
      params.append(param)
    return params
  def apply_fun(params, x, **kwargs):
...
def parallel(*layers):
  ...
  nlayers = len(layers)
  init_funs, apply_funs = zip(*layers)
  def init_fun(rng, inputs):
    rngs = random.split(rng, nlayers)
    return [init(rng, x) for init, rng, x in zip(init_funs, rngs, inputs)]
  def apply_fun(params, inputs, **kwargs):
    rng = kwargs.pop('rng', None)
    rngs = random.split(rng, nlayers) if rng is not None else (None,) * nlayers
    return [f(p, x, rng=r, **kwargs) for f, p, x, r in
            zip(apply_funs, params, inputs, rngs)]
  return init_fun, apply_fun
...
def shape_dependent(make_layer):
  ...
  Args:
    make_layer: a one-argument function that takes an example input as an
      argument and returns an (init_fun, apply_fun) pair.
  ...
  """
  def init_fun(rng, inputs):
    return make_layer(inputs)[0](rng, inputs)
  def apply_fun(params, inputs, **kwargs):
    return make_layer(inputs)[1](params, inputs, **kwargs)
  return init_fun, apply_fun
