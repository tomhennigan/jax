...
def scale_by_adam(b1=0.9, b2=0.999, eps=1e-8):
  ...
  def init_fn(params):
    mu = tree_multimap(jnp.zeros_like, params)  # First moment
    nu = tree_multimap(jnp.zeros_like, params)  # Second moment
    return ScaleByAdamState(count=jnp.zeros([], dtype=jnp.int32), mu=mu, nu=nu)
...
def scale_by_schedule(step_size_fn):
  ...
  def init_fn(_):
    return ScaleByScheduleState(count=jnp.zeros([], dtype=jnp.int32))
...
def add_noise(eta, gamma, seed):
  ...
  def init_fn(_):
    return AddNoiseState(count=jnp.zeros([], dtype=jnp.int32),
